...
date: 2022-12-18 00:30:00 +530
categories: [Write Up. CTF]
tags: [write up, web, ctf] # TAG names should always be lowercase
image: 
   path: /assets/img/ctf/nahamconeu22/head.png
---
...
Upon visitng the given link we come up with a page where there are arithmetic question in a image which we need to solve and submit in a decreasing time limit. Each correct answer will increase your score by 1 and any wrong answer resets it back to 0.

![](/assets/img/ctf/nahamconeu22/2022-12-18_15-38.png)

### Approach
We used Pytesseract to generate strings from images and used eval to solve the equations, We had to grayscale the image and whitelist characters present in a arithmetic equation for better accuracy.
...
### Approach:
We were given a nc command to connect to a machine, There were only limited tools installed into the system, And for the Hell even cd wasn't present. We could not use space character directly as it was getting filtered out. We had to use `${IFS}` instead which is a env variable for space. Though this worked in most cases but was pretty bad. Anorak switched to a python reverse shell as it was installed in the system which made it a bit easier to work around. We found out that the `/etc/passwd` is writable by the user and so we added a new root privileged user and gained root.
...
We were given 2 files ct.enc and a binary file called as program. ct.enc consisted of encrypted text. upon analyzing with ghidra, we found out that it was performing AES 128 encryption on the input. The key for it was generated during the execution. We reused the reversed code from the binary to regereate the key and decrypt our message.
...
### Approach:
We were given source code and URL to a PHP(Apache) based webserver. It had a file upload functionality and it had blacklisted several php files but not `.htaccess` so we used htshells to generate a web shell, uploaded it and obtained the flag.
...
### Approach:
There was a output.txt with encrypted text and the python encryption program. It encrypted 2 bytes at one time. It was just script to bruteforce the LFSR encryption and done.
...
Meanwhile, Here's a eye hurting certificate from the event:
