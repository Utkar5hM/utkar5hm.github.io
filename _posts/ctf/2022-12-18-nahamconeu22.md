...
### Approach
We used Pytesseract to generate strings from ocr images and used eval to solve the equations, We had to grayscale the image and whitelist  certain characters present in a arithmetic equation for better accuracy.
...
### Approach:
We were given a nc command to connect a external machine, There were only limited tools installed into the system, And for the Hell even cd wasn't present. We could not use space character directly as it was getting filtered out. We had to use `${IFS}` instead which is a env variable for space. Though this worked in most cases but was pretty bad. Anorak switched to a python reverse shell as it was installed which made it a bit easier to work around. We found out that the `/etc/passwd` is writable by the user and so we added a new root privileged user and gained root.
...
### Approach:
There was a output.txt with encrypted text and the python encryption program. It encrypted 2 bytes at one time. It was just script writing to decode the LFSR and done.
