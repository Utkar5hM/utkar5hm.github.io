...
Since there's no viable endpoint found, looks like we need to enumerate subdomains. (added the ip with domain siteisup.htb to hosts)

Enumerating subdomains: (since we're looking for something that forbids us, I'm excluding 200 & also bcoz xD all of em were responding with 200.)
...
Doesn't look like extension check can be bypassed: tried `.inc, .phtm` and was able to access the file but as a document.

![](/assets/img/htb/updown/Updown-8.png)

Tried working around by hosting php a file through a python web server and trying to pass it somehow to download and execute via curl. But this felt like a dead end and extremely hard.
...
:-; Tried multiple php rev shells, bind shells or examples to execute shell commands which use proc_open or had that as title, from which none of em worked. php-fpm stuff just went off over my head. :-; cheated a bit, saw a bit of walkthrough and found out that I am on the right path. ;-; We need to manually create the shell from the [php docs](https://www.php.net/manual/en/function.proc-open.php) : 
